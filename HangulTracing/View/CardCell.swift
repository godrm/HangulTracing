//
//  CardCell.swift
//  HangulTracing
//
//  Created by junwoo on 2017. 11. 3..
//  Copyright © 2017년 samchon. All rights reserved.
//

import UIKit
import AVFoundation

class CardCell: UITableViewCell {
  
  var wordLabel: UILabel = {
    let label = UILabel()
    label.backgroundColor = #colorLiteral(red: 0.9385011792, green: 0.7164435983, blue: 0.3331357837, alpha: 1)
    return label
  }()
  var speakerBtn: UIButton = {
    let btn = UIButton()
    btn.setImage(UIImage(named: "smallSpeaker"), for: .normal)
    return btn
  }()
  var speechSynthesizer = AVSpeechSynthesizer()
  
  override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
    super.init(style: style, reuseIdentifier: reuseIdentifier)
    
    contentView.addSubview(wordLabel)
    contentView.addSubview(speakerBtn)
    
    wordLabel.snp.makeConstraints { (make) in
      make.left.equalTo(contentView).offset(20)
      make.right.equalTo(speakerBtn.snp.left).offset(-20)
      make.top.equalTo(contentView).offset(2)
      make.bottom.equalTo(contentView).offset(-2)
    }
    speakerBtn.snp.makeConstraints { (make) in
      make.top.equalTo(contentView).offset(2)
      make.right.equalTo(contentView).offset(-20)
      make.bottom.equalTo(contentView).offset(-2)
      make.width.equalTo(speakerBtn.snp.height)
    }
    speakerBtn.addTarget(self, action: #selector(LetterView.speakerTapped(_:)), for: .touchUpInside)
  }
  
  required init?(coder aDecoder: NSCoder) {
    fatalError("init(coder:) has not been implemented")
  }
  
  func configCell(card: WordCard) {
    wordLabel.text = card.word
    wordLabel.font = UIFont(name: "NanumBarunpen", size: 40)!
  }
  
  @objc func speakerTapped(_ sender: UIButton) {
    synthesizeSpeech(fromString: wordLabel.text!)
  }
  
  func synthesizeSpeech(fromString string:String) {
    let speechUtterence = AVSpeechUtterance(string: string)
    speechSynthesizer.speak(speechUtterence)
  }
  
}
